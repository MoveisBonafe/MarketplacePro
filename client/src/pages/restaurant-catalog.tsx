...
export default function RestaurantCatalog() {
  ...
  useEffect(() => {
    if (!user || user.userType !== 'restaurante') {
      setLocation('/login');
    }
  }, [user, setLocation]);
  ...
  const { data: pricingTables } = useQuery<PricingTable[]>({
    queryKey: ['/api/pricing-tables', 'restaurante'],
    queryFn: async () => {
      const response = await fetch('/api/pricing-tables?userType=restaurante');
  ...
  const { data: announcements } = useQuery<Announcement[]>({
    queryKey: ['/api/announcements', 'restaurante'],
    queryFn: async () => {
      const response = await fetch('/api/announcements?userType=restaurante');
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              ...
              <div>
                <h1 className="text-xl font-semibold text-gray-900">Catálogo Restaurante</h1>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8 p-4 bg-amber-50 border border-amber-200 rounded-lg">
          ...
          <p className="text-amber-700 mt-2 text-sm">
            Como usuário restaurante, você tem acesso ao preço especial com desconto exclusivo.
          </p>
        </div>
